...
"""
var_value: the explicit regressors X^{(r)}
var_dis: the candidate split variables X^{(s)}
var_res: the response variable Y
"""
class Tree(object):
    ...
    def lookup(self, x):
        """Returns the predicting beta parameter.
           here we require that x is row of the dataframe, a Series with column names"""
        if self.left == None:
            # var_value could only include continuous(numeric) variables
    ...
    def prune_tree(self):
        ...
        while 1:
            ...
            # to deal with numerically approximation error
            if alpha < 0:
                alpha = 0
            for node in nodes:
                node.right = None
...
def grow_tree(data, id_value_all, id_cand_all, id_dis, id_res, index, labels = [], depth = 1, \
              max_depth = 20, Nmin = 50,  start = False, feat_bag = False,\
    ...
    #print 'Nmin is', Nmin
    'for debug ----------------------------'
    # feat_bag=True for trees in random forests, which means
    # [1]bootstrap resampling; [2]randomly choose p^{mi} variables in each node, mi: tunning parameter.
    ...
    # the minimal sample size Nmin should be at least larger than 2(num_vars+1）
    ...
    split_var,split_val = get_split(data, id_cand, id_value, id_dis, labels, residual, Nmin = Nmin, sNsplit = sNsplit)
    'debug ---------------------------'
    ...
    'debug ---------------------------'
    if split_var not in list(var_dis):
...
def cvt(data, id_value_all, id_cand_all, id_dis, id_res, v, labels = [], max_depth = 500, Nmin = 50, sNsplit = 5, mi=0.5, beta=0):
    """Grows regression tree using v-fold cross validation.
     ...
     beta is to adjust: choose the minimal alpha when the corresponding mse < mse_min + beta* std_err """
    full_tree = grow_tree(data, id_value_all, id_cand_all, id_dis, id_res, [1], labels = labels, \
                          start = True, Nmin = Nmin, sNsplit = sNsplit,mi=mi )
    ...
    for i in range(len(lv_s)):
        ...
        test_vs.append(test)
    # choose the subtree that has the minimum cross-validated errors estimate
    min_R = float("Inf")
    # the standard error of mses
    min_err = 1000
    min_ind = 0
...
def get_split(data, id_cand, id_value, id_dis, labels, residual, Nmin ,sNsplit):
    ...
    # select the var_split that has candidate splits, cause in some cases there are no proper split levels
    var_split_real=[]
    ...
    # criterion: the sum of the absolute values of kendall's taus
    criterion = {}
    # best_split: the best_split for each candidate split variable
    best_split={}
    for i in kendall_tau_total:
        criterion[i], best_split[i]= criterion_func(kendall_tau_total[i],split_dict[i])
    '----different from v1--------'
    # 3. select id_split_best
    best_split_variable =  max(criterion, key = criterion.get)
    best_split_value = best_split[best_split_variable]
    if criterion[best_split_variable]==float('-inf'):
        best_split_variable=None
        best_split_value=None
        return best_split_variable, best_split_value
    return best_split_variable, best_split_value
...
def criterion_func(taus_dict,split_val):
    taus_sum={}
...
def region_reg_res(data, var_value, var_res):
    """Calculates sum of squared error for some node in the regression tree.
       and return sse, beta(including the intercept term),sigma_hat, y_fit and residuals"""
    nvar = len(var_value)
    nrow = len(data)
    ...
    sigma = math.sqrt(sse/(nrow-nvar-1)) # sigma is a float type
...
def paired_power_set(s):
    ...
    n = len(s)
    test_marks = [1<<i for i in range(0,n)] # <<i 相当于 *2^{i-1}
    ...
    for k1, k2 in pair:
        ...
        for idx, item in enumerate(test_marks): # enumerate returns a iterator
            ...
            if k2&item:
                l2.append(s[idx])
        yield [set(l1),set(l2)]
def set2str(splitset):
    if len(splitset)==1:
        return '{ '+str(list(splitset)[0])+' }'
    else:
        return '{ '+reduce(lambda x,y: str(x)+', '+str(y), splitset)+' }'
