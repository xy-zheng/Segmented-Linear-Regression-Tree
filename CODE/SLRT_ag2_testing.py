...
def grow_tree(data, id_value_all, id_cand_all, id_dis, id_res, index, labels = [], depth = 1, \
    ...
    if split_val == None:
        return root
    stopping = if_stop(residual, data, var_value, var_dis, split_var, split_val, Nmin)
    'debug ---------------------------'
...
def get_split(data, id_cand, id_value, id_dis, labels, residual, Nmin ,sNsplit, Nsplit):
    ...
    # 2.2 we use split_dict_more[final var] to choose split_val
    final_res1=[]
...
def if_stop(residual, data, var_value, var_dis, split_var, split_val, Nmin):
    if_stop_value = 1
    data.loc[:,'residual'] =residual
    pvalues=kendall_calculate_pvalues(split_var, split_val, var_dis, var_value, data, Nmin)
    if(min(pvalues)<(0.05/len(pvalues))):
        if_stop_value=0
