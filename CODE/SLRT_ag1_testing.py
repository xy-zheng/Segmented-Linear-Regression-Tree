...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun 22 11:33:56 2019
...
def grow_tree(data, id_value_all, id_cand_all, id_dis, id_res, index, labels = [], depth = 1, \
    ...
    stopping = if_stop(residual, data, var_value, var_dis, split_var, split_val, Nmin, sNsplit)
    'debug ---------------------------'
...
def if_stop(residual, data, var_value, var_dis, split_var, split_val, Nmin, sNsplit):
    if_stop_value = 1
    data.loc[:,'residual'] =residual
    pvalues=kendall_calculate_pvalues(split_var, split_val, var_dis, var_value, data, Nmin)
    if(min(pvalues)<(0.05/(len(pvalues)*sNsplit))):
        if_stop_value=0
...
#    N_candidate_splits = 0
    var_split_real=[]
    for var in split_dict:
        if(len(split_dict[var])>0):
            var_split_real.append(var)
#            N_candidate_splits = N_candidate_splits + len(split_dict[var])
