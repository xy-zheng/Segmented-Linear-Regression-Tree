...
def grow_tree(data, id_value_all, id_cand_all, id_dis, id_res, index, labels = [], depth = 1, \
    ...
    "Nsplit" to be passed in the function get_split_val, the maximum number of candidate split levels to be investigated；
    ...
    #print 'Nmin is', Nmin
    'for debug ----------------------------'
    num_splitvars = len(id_cand_all) # number of candidate split variables
    num_vars = len(id_value_all) # number of regressors
    if (feat_bag):
    ...
    # sample size should be at least larger than 2*(num_vars+1)
    ...
    split_var,split_val = get_split_var(data, id_cand, id_value, id_dis, labels, residual, Nmin = Nmin, Nsplit = Nsplit)
    if split_val == None:
        return root
    ...
    root.left = grow_tree(data1, id_value_all, id_cand_all, id_dis, id_res, index, labels = labels,\
                          depth = depth + 1, max_depth = max_depth, Nmin = Nmin, \
                           Nsplit = Nsplit, feat_bag = feat_bag, region_info = region_info_left,mi = mi)
    index[0] = index[0]+1
    if split_var not in var_dis:
    ...
    root.right = grow_tree(data2, id_value_all, id_cand_all, id_dis, id_res, index, labels = labels,\
                          depth = depth + 1, max_depth = max_depth, Nmin = Nmin, \
                           Nsplit = Nsplit, feat_bag = feat_bag, region_info = region_info_right,mi=mi)
    return root

def cvt(data, id_value_all, id_cand_all, id_dis, id_res, v, labels = [], max_depth = 500, Nmin = 50, Nsplit = 10, mi=0.5, beta=0):
    """
    the tree pruning function, v is the parameter for cross-validation in choosing alpha
    """
    ...
    for i in range(len(full_t)):
        ...
        # print the penalizing parameter, the alpha here is different from the one in paper,but equivalent.
        print i,'th', 'alpha is',ak/data.shape[0], 'error is ',R_k_mean
        if (R_k_mean <= min_R+beta*min_err):
            min_R = R_k_mean
            min_err = R_k_error
            min_ind = i
    print 'alpha_star is', a_s[min_ind]/data.shape[0]
...
def get_split_var(data, id_cand, id_value, id_dis, labels, residual, Nmin ,Nsplit):
    ...
    # 1.1 get candidate split variable and level pairs => split_dict
    ## detailed info.
    ## for continuous split variable, we limit the split levels starting from Nmin to len-Nmin
    ...
    # select the var_split that has candidate splits, since some split variables have no suitable split levels
    var_split_real=[]
    ...
    # 1.2 get kendall_tau_total: C(j, a) for all candidate split variable and level pairs, time consuming step
    var_value = labels[id_value]
    kendall_tau_total = {}
    ...
    for split_var in var_split_real:
        for split_val_id in range(len(split_dict[split_var])):
            ...
            taus_split=kendall_one_split(split_var, split_val, var_dis, var_value, data, Nmin)
            kendall_tau_total[split_var].update({split_val_id:taus_split})
    # 2. calculate the criterion and choose the best split value for each candidate split variable
...
def kendall_one_split(split_var, split_val, var_dis, var_value, data, Nmin):
    taus=[]
    if split_var in var_dis:
        id1 = data[split_var].isin(split_val[0])
...
# return the best split level for a fixed split variable, and its corresponding criterion function
...
def get_split_dict(var_split, var_dis, data, Nsplit, Nmin):
    ...
    for var in var_split:
        if var not in var_dis:
            ...
            if(len(x_split_unique)<Nsplit):
                split_range = x_split_unique
            else:
                step = len(id_mid)/Nsplit
                if step == 0:
                    step = 1
            ...
            if len(split_range_all)>2*Nsplit:
                split_range = random.sample(split_range_all, 2*Nsplit)
            else:
...
def data_partition(tree, data):
    ...
    for l in range(len(leaf_list)):
        data_partitions[leaf_list[l]] = data[data['leaf']==leaf_list[l]].copy()
