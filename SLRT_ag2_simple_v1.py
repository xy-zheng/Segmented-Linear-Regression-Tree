...
def grow_tree(data, id_value_all, id_cand_all, id_dis, id_res, index, labels = [], depth = 1, \
    ...
    split_var,split_val = get_split(data, id_cand, id_value, id_dis, labels, residual, Nmin = Nmin, sNsplit = sNsplit, Nsplit=Nsplit)
    'debug ---------------------------'
...
def cvt(data, id_value_all, id_cand_all, id_dis, id_res, v, labels = [], max_depth = 500, Nmin = 50, Nsplit = 10, sNsplit = 5, mi=0.5, beta=0):
    ...
    lv_s = [list(range(i, len(data), v)) for i in range(v)]
...
def get_local_split(split_var0, split_val0, var_split_real, split_dict, data, var_value, var_dis, Nmin):
    ...
    # Pdiag(m, a)return a matrix P such that PmP^{T} and PmP^{T} are both diagonal
    mdiag=Pdiag(mat,mat1)
    data_temp=data.copy()
