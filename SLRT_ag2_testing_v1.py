...
def grow_tree(data, id_value_all, id_cand_all, id_dis, id_res, index, labels = [], depth = 1, \
    ...
    """Nsplit and sNsplit are to be passed in get_split_var"""
    ...
    split_var,split_val, num_candidate_splits = get_split(data, id_cand, id_value, id_dis, labels, residual, Nmin = Nmin, sNsplit = sNsplit, Nsplit=Nsplit)
    'debug ---------------------------'
...
def get_split(data, id_cand, id_value, id_dis, labels, residual, Nmin ,sNsplit, Nsplit):
    ...
    # split_dict is the coarse one, and split_dict_more is more detailed
    split_dict = get_split_dict(var_split, var_dis, data, sNsplit, Nmin)
    split_dict_more = get_split_dict(var_split, var_dis, data, Nsplit, Nmin)
    num_candidate_splits = sum([len(split_dict_more[i]) for i in split_dict_more.keys()])
    # select the var_split that has candidate splits
    var_split_real=[]
    for var in split_dict_more:
        if(len(split_dict_more[var])>0):
            var_split_real.append(var)
    var_split_real=numpy.array(var_split_real)
    # 2.0 if there are exactly no proper splits/that satisfy the sample size restriction, return
    if len(var_split_real)==0:
        return best_split_variable, best_split_value,num_candidate_splits
    # 2.1 we use split_dict to choose split_var firstly, split_val will also be generated but we dont use it as the final output
    # split_dict is quite coarse, to save computation; once split_var is fixed, split_val will be selected with more scrutiny on split_dict_more
    # iterate on all split vars and split values in split_dict: obtain the optimal splits
    ...
    for split_var0 in var_split_real:
        for split_val0 in split_dict[split_var0]:
            best_split_var, best_split_val, best_tau=get_local_split(split_var0, \
    ...
    if(len(eq_res)==0):
        ...
        final_var, temp_val=get_matched(final_res_df, var_dis)
    # 2.2 we use split_dict_more[final_var] to choose split_val
    final_res1=[]
    for split_val0 in split_dict[final_var]:
        best_split_var, best_split_val, best_tau=get_local_split(final_var, \
                   split_val0, var_split_real, split_dict_more, data, var_value, var_dis, Nmin)
        final_res1.append((split_var0, split_val0, best_split_var, best_split_val, best_tau))
    final_res1_df=pd.DataFrame(final_res1, columns=['var0', 'val0', 'var1', 'val1', 'tau'])
    final_var, final_val=get_matched(final_res1_df, var_dis)
    return final_var, final_val,num_candidate_splits
"""for correlated one"""
def get_local_split(split_var0, split_val0, var_split_real, split_dict, data, var_value, var_dis, Nmin):
...
def get_matched(final_res_df, var_dis):
    ...
    if len(eq_var_con)>0:
        ...
        eq_var_con.loc['diff_ratio'].apply(pd.to_numeric, errors = 'coerce')
...
def diag(m, a):
    ...
    return c
"""below are the function for hypothesis testing: if stop split"""
def if_stop(residual, data, var_value, var_dis, split_var, split_val, Nmin, num_candidate_splits):
    ...
    if(min(pvalues)<(0.05/(len(pvalues)* num_candidate_splits))):
        if_stop_value=0
...
def get_split_dict(var_split, var_dis, data, sNsplit, Nmin):
    ...
    for var in var_split:
        if var not in var_dis:
            ...
            split_dict.update({var:split_range})
    return split_dict
"""get kendall's tau coefficients"""
def sig_number(l):
    l = numpy.array(l)
